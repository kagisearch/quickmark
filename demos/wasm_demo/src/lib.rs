...
pub fn main() {
    ...
    crate::plugins::extra::add(&mut parser);
    ...
    let do_render = Closure::<dyn FnMut()>::new(move || {
        ...
        ast.walk(|node, depth| {
            ...
            if let Some(data) = node.cast::<crate::mdparser::inline::Text>() {
                ast_str.push_str(&format!("{}: {:?}\n", name, data.content));
            } else if let Some(data) = node.cast::<crate::mdparser::inline::TextSpecial>() {
                ast_str.push_str(&format!("{}: {:?}\n", name, data.content));
            } else {
                ast_str.push_str(&format!("{}\n", name));
            }
