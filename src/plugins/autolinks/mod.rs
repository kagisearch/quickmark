...
//! quickmark::plugins::autolinks::add(md);
//!
//! assert_eq!(
...
use crate::{
    mdparser::inline::{InlineRule, InlineState, TextSpecial},
...
pub fn check_prev_char(prev: char) -> bool {
    matches!(prev, ' ' | '\t' | '\r' | '\n' | '*' | '_' | '~' | '(')
}

/// Check if the preceding syntax allows an autolink.
fn check_preceding(state: &InlineState, bscan_len: usize) -> Option<()> {
    if state.pos - bscan_len > 0
        && !check_prev_char(state.src.chars().nth(state.pos - bscan_len - 1)?)
    {
        return None;
    }
    Some(())
