...
///
/// ```rust
/// use quickmark::mdparser::block::builtin::BlockParserRule;
/// use quickmark::mdparser::core::{CoreRule, Root};
/// use quickmark::plugins::cmark::block::reference::{ReferenceMap, DefaultReferenceMap, CustomReferenceMap};
/// use quickmark::{MarkdownIt, Node};
///
...
/// ```rust
/// use quickmark::mdparser::core::{CoreRule, Root};
/// use quickmark::plugins::cmark::block::reference::{ReferenceMap, DefaultReferenceMap};
/// use quickmark::{MarkdownIt, Node};
///
/// let md = &mut MarkdownIt::new();
/// quickmark::plugins::cmark::add(md);
...
impl BlockRule for ReferenceScanner {
    ...
    fn run(state: &mut BlockState) -> Option<(Node, usize)> {
        ...
        loop {
            match chars.next() {
                // NOTE(Rehan): this branch was `pos += 1`, but seems unused afterwards
                Some(' ' | '\t') => {}
                Some('\n') | None => break,
